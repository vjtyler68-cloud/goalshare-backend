...
const createModule = (moduleName) => {
    const baseDir = path_1.default.join(__dirname, '../', 'app', 'modules', moduleName);
    const files = [
...
import { ${capitalize(moduleName)}Validation } from "./${moduleName}.validation";

const router = express.Router();

router.get("/", ${capitalize(moduleName)}Controller.getAll${capitalize(moduleName)});
router.get("/:id", ${capitalize(moduleName)}Controller.get${capitalize(moduleName)}ById);

router.post(
  "/",
  validateRequest.body(${capitalize(moduleName)}Validation.create${capitalize(moduleName)}ZodSchema),
  ${capitalize(moduleName)}Controller.createIntoDb
);

router.patch(
  "/:id",
  validateRequest.body(${capitalize(moduleName)}Validation.update${capitalize(moduleName)}ZodSchema),
  ${capitalize(moduleName)}Controller.updateIntoDb
);

router.delete("/:id", ${capitalize(moduleName)}Controller.deleteIntoDb);
router.delete("/soft/:id", ${capitalize(moduleName)}Controller.softDeleteIntoDb);
...
const getAll${capitalize(moduleName)} = catchAsync(async (req: Request, res: Response) => {
  const result = await ${capitalize(moduleName)}Services.getAll${capitalize(moduleName)}(req.query);
  sendResponse(res, {
...
const get${capitalize(moduleName)}ById = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await ${capitalize(moduleName)}Services.get${capitalize(moduleName)}ByIdFromDB(id);
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
...
const updateIntoDb = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await ${capitalize(moduleName)}Services.updateIntoDb(id, req.body);
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
